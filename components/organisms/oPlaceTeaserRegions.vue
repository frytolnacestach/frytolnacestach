                ...
                <div class="o-place-teaser-regions__outer">
                    <div class="o-place-teaser-regions__inner">
                        <div class="o-place-teaser-regions__items">
                            <div class="o-place-teaser-regions__item" v-for="spot in spots" :key="spot.id">
                                <div class="o-place-teaser-regions__content">
                                    <div class="o-place-teaser-regions__image loading-image -green">
                                        <div v-if="images && images.find(image => image.id === spot.id_image_hero)" class="o-place-teaser-regions__image-lazyload">
                                        ...
                                        <NuxtLink class="o-place-teaser-regions__image-link" :to="`/svet/misto/${spot.slug}`" :aria-label="`Čti více o místě ${spot.name}`"></NuxtLink>
                                    </div>
                                    <div class="o-place-teaser-regions__text">
                                        <nuxtLink class="o-place-teaser-regions__name" :to="`/svet/misto/${spot.slug}`" :aria-label="`Čti více o místě ${spot.name}`">{{ spot.name }}</nuxtLink>
                                        <div class="o-place-teaser-regions__wysiwyg" v-if="spot.information_chatgpt" v-html="spot.information_chatgpt.slice(0, spot.information_chatgpt.lastIndexOf(' ', 160)).replace(/<\/?[^>]+(>|$)/g, '')"></div>                               
                                    </div>
                                </div>
                            </div>
                        </div>
...
<script>
    ...
    export default {
        ...
        async fetch() {
            ...
            if(this.type === "state") {
                this.spots = await fetch(`https://api.frytolnacestach.cz/api/places-regions-id-state/${this.IDplace}`).then((res) => res.json());
            }
