                        ...
                        <div class="t-grid -place-main-with-aside">
                            <div class="t-grid__section -main">
                                ...
                                <section class="t-section mt-2 mb-4" v-if="place && place.length > 0 && place[0].information_author?.length > 0">
                                    <div class="t-section__inner">
                                        <oInformationBlock :title="'O městě ' + (place[0].name ? place[0].name : '')" :perexWysiwyg="place[0].information_author[0].text" :authorID="place[0].information_author[0].author_create" styleThema=" -green" />
                                    </div>
                                </section>
                                ...
                                <oPlaceTeaserList :headline="'Co vidět ve městě ' + place[0].name" :IDplace="place[0].id" type="spots" typePage="city" styleGap=" my-4" v-if="place && place.length > 0" />
                                ...
                                <!-- SECTION - Review -->
                                <oReviewItem :account="account" :IDplace="place[0].id" type="city" v-if="place && place.length > 0 && place[0].id" />
                                <!-- SECTION - Review END -->
                            ...
                            <div class="t-grid__section -aside-place-status">
                                ...
                                <section class="t-section -px-world my-1" v-if="place && place.length > 0 && place[0].id">
                                    <div class="t-section__inner">
                                        <oVisitedButton :account="account" :place="place[0].id" placeType="city" />
                                    </div>
                                </section>
                            ...
                            <div class="t-grid__section -aside-content">
                                ...
                                <oSidebarParking :items="this.place[0].parking" v-if="place && place.length > 0 && place[0].parking && this.place[0].parking.length !== 0" />
                                <!-- SECTION - Parking - sidebar - END -->

                                <!-- SECTION - Events - sidebar -->
                                <oSidebarEvent :place="place[0].id" type="city" v-if="place && place.length > 0 && place[0].id" />
                    ...
                    <div class="t-section__inner">
                        <div class="t-grid -world-ful">
                            <div class="t-grid__section -content">
                                ...
                                <!-- SECTION - města - all -->
                                <section class="t-section -p0 -py4 -px-world-big -h-scroll" v-if="placeState && placeState.length > 0">
                                    <div class="t-section__inner">
    ...
    export default defineComponent({
            ...
            return {
                ...
                mNavBreadcrumbsPlaceArray: [
                    ...
                    },
                    {
                        ...
                        icon: false,
                        type: "city",
                        name: (this.place && this.place.length > 0) ? this.place[0].name : "Město",
                        url: (this.place && this.place.length > 0) ? ("/svet/mesto/" + this.place[0].slug) : "/svet/mesto",
                        status: "span"
                    }
                ],
                oHotInfoHeroArray: [
        ...
        methods: {
            async fetchData() {
                ...
                // PAGE - Spot detail
                // Place
                const responsePlace = await fetch(`https://api.frytolnacestach.cz/api/places-city/${route.params.slug}`)
                ...
                if (this.place && this.place.length > 0 && this.place[0].id_state !== null && this.placeState[0] && this.placeState[0].id_image_cover !== null ) {
                    ...
                    this.imageState = await responseImagePlaceState.json()
                }
                // PlaceContinent
                if (this.place && this.place.length > 0) {
                ...
                // PlaceCity
                if (this.place && this.place.length > 0) {
                    const responsePlaceCity = await fetch(`https://api.frytolnacestach.cz/api/places-cities-id-state/${placeState[0].id}?showType=list`)
                    this.placeCity = await responsePlaceCity.json()
                }
                // Images
                let imagesCities = null
                ...
                if (this.place && this.place.length > 0 && imagesPlacesCitiesID  !== nulll ) {
                    const responseImagePlaceCity = await fetch(`https://api.frytolnacestach.cz/api/images-array?id=${imagesPlacesCitiesID.join(',')}`)
                ...
                if (this.place && this.place.length > 0) {
                    // Meta
                    this.headMeta.title = `${(this.place && this.place.length > 0 && this.place[0].name) ? this.place[0].name : 'Město'} | Cestovatelský portál Frytol na cestách`
                    ...
                    if (this.place[0].seo_tags && this.place[0].seo_tags.length > 0) {
                        ...
                        this.headMeta.keywords = (this.place[0].name ? this.place[0].name : '') + metaSeoTags + ', město, cestování, svět, cestovatelský portál, jaké další města tu jsou, plánování cesty, dovolená, pravidla, kultura, ceny'
                    } else {
                        this.headMeta.keywords = (this.place[0].name ? this.place[0].name : '') + ', město, cestování, svět, cestovatelský portál, jaké další města tu jsou, plánování cesty, dovolená, pravidla, kultura, ceny'
                    }
                    ...
                    this.headMeta.ogTitle = `${(this.place && this.place.length > 0 && this.place[0].name) ? this.place[0].name : 'Město'} | Cestovatelský portál Frytol na cestách`
                    this.headMeta.ogDescription = (this.place[0].information_author?.length > 0 ? this.place[0].information_author[0].text.replace(/<\/?[^>]+(>|$)/g, '').slice(0, this.place[0].information_author[0].text.lastIndexOf(' ', 160)) : this.place[0].information_chatgpt ? this.place[0].information_chatgpt.replace(/<\/?[^>]+(>|$)/g, '').slice(0, this.place[0].information_chatgpt.lastIndexOf(' ', 160)) : this.place[0].name ? this.place[0].name : 'Město')
                    this.headMeta.ogUrl = `https://www.frytolnacestach.cz/svet/mesto/${this.place[0].slug}`
                    ...
                    this.headScript.image = ((this.imagePlace && imagePlace.length > 0 && this.imagePlace[0].id) ? ("https://image.frytolnacestach.cz/storage/world/cities/" + this.imagePlace[0].name + ".webp") : "" )
            ...
            async loadPosts() {
                ...
                //load posts
                if (this.place && this.place.length > 0) {
                    const responsePosts = await fetch(`https://api.frytolnacestach.cz/api/posts-id-city/${this.place[0].id}?showType=list&page=${this.postsPage}&items=${this.postsPerPage}`)
            ...
            async loadVideos() {
                ...
                //load videos
                if (this.place && this.place.length > 0) {
                    const responseVideo = await fetch(`https://api.frytolnacestach.cz/api/videos-id-city/${this.place[0].id}?showType=list&page=${this.videosPage}&items=${this.videosPerPage}`)
