                                            ...
                                            <div class="t-grid -place-main-with-aside">
                                                <div class="t-grid__section -main">
                                                    ...
                                                    <section class="t-section -p0 -px-world my-2">
                                                        <div class="t-section__inner">
                                                            <MoleculesHeadline title="Sousední státy státu " :titleValue="place[0].name" perex="Přehled o zemích, které sdílí pevninskou či mostní hranici s touto destinací, představujeme kulturu, zajímavosti a potenciální cestovatelská dobrodružství." styleThema=" -world" styleAlign=" -p-left" styleGap=" mb-2" v-if="placesStatesNeighboring && placesStatesNeighboring.length > 0" />
                                                            <MoleculesHeadline title="Sousední státy státu " :titleValue="place[0].name" :perex="'Stát ' + place[0].name + ' nemá žádné přímé sousední státy'" styleThema=" -world" styleAlign=" -p-left" styleGap=" mb-2" v-else />
                                                            <OrganismsCoverNeighboring :items="placesStatesNeighboring" :images="imagesStatesNeighboring" v-if="placesStatesNeighboring && placesStatesNeighboring.length > 0" />
                                                        </div>
                                                    </section>
...
<script setup>
    ...
    let preTitle = ''
    let activeTab = 'sousedni-staty'
    let activeTabName = 'Sousední státy'
    let showHero = true
    // DATA API
    const place = ref([])
    const imagePlace = ref([])
    const placeContinent = ref([])
    const placesStatesNeighboring = ref([])
    const imagesStatesNeighboring = ref([])
    const tabsLoad = ref([])
    // DATA OTHER
    let tabs = [
    ...
    const loadData = async () => {
        ...
        if (place.value && place.value.length > 0 && place.value[0].ids_neighboring_countries !== null) {
            const idsNeighboringCountries = place.value[0].ids_neighboring_countries.map(item => item.id)
            const placesStatesNeighboringResponse = await $fetch(`https://api.frytolnacestach.cz/api/places-states-array?showType=list&id=${idsNeighboringCountries.join(',')}`)
            const placesStatesNeighboringData = JSON.parse(placesStatesNeighboringResponse) || []
            placesStatesNeighboring.value = placesStatesNeighboringData

            if (placesStatesNeighboring.value) {
                const imagesplaceStatesNeighboringID = placesStatesNeighboring.value.map(placesStateNeighboring => placesStateNeighboring.id_image_cover).filter(id => id !== null && id !== '')
                if (imagesplaceStatesNeighboringID) {
                    const imagesStatesNeighboringResponse = await $fetch(`https://api.frytolnacestach.cz/api/images-array?id=${imagesplaceStatesNeighboringID.join(',')}`)
                    const imagesStatesNeighboringData = JSON.parse(imagesStatesNeighboringResponse) || []
                    imagesStatesNeighboring.value = imagesStatesNeighboringData
                }
            }
        }
        ...
        if (place.value && place.value.length > 0) {
            ...
            // Meta
            headMeta.title = `${tabLabel} státu ${place.value[0].name || 'Stát'} | Cestovatelský portál Frytol na cestách`
            ...
            headMeta.keywords = ((place.value[0].seo_tags && place.value[0].seo_tags.length > 0) ? (place.value[0].name || '') + ', ' + place.value[0].seo_tags.map(item => item.tag).join(', ') + ', stát, sousední státy, cestování, svět, cestovatelský portál, která města tu jsou, plánování cesty, dovolená, pravidla cesty, o státu' : (place.value[0].name || '') + ', stát, sousední státy, cestování, svět, cestovatelský portál, která města tu jsou, plánování cesty, dovolená, pravidla cesty, o státu')
            ...
            headMeta.ogTitle = `${tabLabel} státu ${place.value[0].name || 'Stát'} | Cestovatelský portál Frytol na cestách`
