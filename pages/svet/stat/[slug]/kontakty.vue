...
<script setup>
    ...
    let preTitle = ''
    let activeTab = 'kontakty'
    let activeTabName = 'Kontakty'
    let showHero = true
    // DATA API
    const place = ref([])
    ...
    const loadData = async () => {
        ...
        if (place.value && place.value.length > 0) {
            ...
            // Meta
            headMeta.title = `Důležité ${tabLabel} ve státě ${(place && place.value.length > 0 && place.value[0].name) ? place.value[0].name : 'Stát'} | Cestovatelský portál Frytol na cestách`
            ...
            headMeta.ogTitle = `Důležité ${tabLabel} ve státě ${(place && place.value.length > 0 && place.value[0].name) ? place.value[0].name : 'Stát'} | Cestovatelský portál Frytol na cestách`
        ...
        // SET PAGE
        preTitle = `Důležité ${activeTabName} ve státě`
    }
